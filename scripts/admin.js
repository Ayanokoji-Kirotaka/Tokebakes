...
class DataSyncManager {
  ...
  init() {
    if (window.TokeUpdateSync) {
      ...
      recordAdminLog("sync", "Connected to shared update bus");
    }
  ...
  notifyDataChanged(operationType, itemType, extra = {}) {
    ...
    if (this.syncBus && typeof this.syncBus.publishDataUpdate === "function") {
      recordAdminLog("sync", "Publishing admin change", {
        operationType,
        itemType: normalizedType,
        version: extra?.contentVersion || 0,
      });
...
function setAdminSyncBadge(state, detail = {}) {
  ...
  if (state === "syncing") {
    ...
    recordAdminLog("sync", "Sync in progress", {
      version: Number(detail?.contentVersion || detail?.appliedVersion || 0),
      changeType: detail?.changeType || detail?.lastChangeType || "all",
    });
    return;
  }
  ...
  if (state === "error") {
    ...
    recordAdminLog("sync", "Sync failed", detail || {});
    return;
  }
  ...
  recordAdminLog("sync", "Sync ready", {
    version,
    mode: detail?.mode || "",
...
function recordAdminError(type, message, details = null) {
  ...
  recordAdminLog(type, message, details);
}
...
async function clearThemeCachesSafely() {
  try {
    recordAdminLog("cache", "Clearing theme cache");
...
async function clearAppCachesSafely() {
  try {
    recordAdminLog("cache", "Clearing application caches");
...
async function loadStatsPanel(force = false) {
  ...
  recordAdminLog("stats", force ? "Refreshing diagnostics panel" : "Loading diagnostics panel");
  ...
  try {
    ...
    recordAdminLog("stats", "Diagnostics refreshed", { days });
  } catch (error) {
...
async function refreshAdminUiFromSync(changeType = "all", payload = {}) {
  ...
  recordAdminLog("sync", "Applying synced admin refresh", {
    changeType,
    version: Number(payload?.contentVersion || 0),
  });
...
function renderDebugActivityLog() {
  const body = document.getElementById("debug-logs-body");
  if (!body) return;
  const rows = adminDiagnosticsState.logs.slice(0, ADMIN_ACTIVITY_LOG_LIMIT);
  if (!rows.length) {
  ...
  body.innerHTML = rows
    .map(
      (row) => `
        <tr>
          <td>${escapeHtml(formatDebugDate(row.ts))}</td>
...
function clearAdminDiagnosticsLogs() {
  adminDiagnosticsState.logs = [];
  adminDiagnosticsState.errors = [];
  renderDebugActivityLog();
  renderDebugErrorLog();
  setDebugStatusLine("Diagnostics logs cleared.");
}
...
function calculateDiscountPreviewMeta(originalPrice, discountedPrice) {
  const original = toMoney(originalPrice, 0);
  const discounted = toMoney(discountedPrice, 0);
  if (
...
function updateSpecialsDiscountPreview() {
  const preview = document.getElementById("specials-discount-preview");
  if (!preview) return null;

  const hiddenSpecialFieldsRow = document
    .getElementById("specials-special-flag")
    ?.closest(".form-row");
  if (hiddenSpecialFieldsRow) {
    hiddenSpecialFieldsRow.style.display = "none";
  }

  const originalField = document.getElementById("specials-original-price");
  const discountedField = document.getElementById("specials-price");
  ...
  const originalRaw = toSafeString(originalField?.value).trim();
  const discountedRaw = toSafeString(discountedField?.value).trim();

  const discountMeta = calculateDiscountPreviewMeta(
    originalRaw === "" ? null : originalRaw,
    discountedRaw === "" ? null : discountedRaw,
  );

  preview.classList.toggle("is-active", Boolean(discountMeta));
  if (!discountMeta) {
    if (specialFlagField) specialFlagField.value = "false";
    if (badgeTextField) badgeTextField.value = "";
    if (badgeIconField) badgeIconField.value = "fire";
    preview.innerHTML = `
      <strong>No discount yet</strong>
      <span>Enter both prices to generate the live badge and comparison.</span>
    `;
    return null;
  }

  if (specialFlagField) specialFlagField.value = "true";
  if (badgeTextField) badgeTextField.value = `${discountMeta.percent}% OFF`;
  if (badgeIconField) badgeIconField.value = "fire";

  preview.innerHTML = `
    <strong>-${escapeHtml(String(discountMeta.percent))}% OFF</strong>
    <span>Was \u20A6${escapeHtml(formatPrice(discountMeta.original))}</span>
    <span>Now \u20A6${escapeHtml(formatPrice(discountMeta.discounted))}</span>
  `;
  return discountMeta;
}
...
function buildSpecialsAdminCardElement(item = {}, badgeOrder = null) {
  ...
  const discountMeta = calculateDiscountPreviewMeta(originalPrice, price);
  ...
  card.innerHTML = `
    ...
    <div class="specials-admin-overlay">
      ${
        discountMeta
          ? `<p><strong>Badge:</strong> -${escapeHtml(String(discountMeta.percent))}% OFF</p>`
          : ""
      }
      <p><strong>Title:</strong> ${escapeHtml(title)}</p>
      <p><strong>Price:</strong> \u20A6${escapeHtml(formatPrice(price))}</p>
      ${
        discountMeta
          ? `<p><strong>Original:</strong> <span style="text-decoration:line-through;opacity:.75;">\u20A6${escapeHtml(
              formatPrice(discountMeta.original),
            )}</span></p>`
          : ""
      }
...
async function saveSpecialsItem(e) {
  ...
  const discountMeta = calculateDiscountPreviewMeta(originalPrice, priceValue);
  if (originalPriceRaw !== "" && !discountMeta) {
    setFieldError(
      originalPriceField,
  ...
  const actionResult = await runAdminAction({
    ...
    task: async (progress) => {
      ...
      const formData = {
        ...
        is_special: Boolean(discountMeta),
        badge_right_text: discountMeta ? `${discountMeta.percent}% OFF` : "",
        badge_right_icon: "fire",
...
function setupEventListeners() {
  ...
  const debugClearLogsBtn = document.getElementById("debug-clear-logs-btn");
  if (debugClearLogsBtn) {
    debugClearLogsBtn.addEventListener("click", () => {
      clearAdminDiagnosticsLogs();
      showNotification("Diagnostics logs cleared.", "success");
    });
  }
  ...
  const specialsPriceField = document.getElementById("specials-price");
  const specialsOriginalPriceField = document.getElementById(
    "specials-original-price",
  );
  [specialsPriceField, specialsOriginalPriceField]
    .filter(Boolean)
    .forEach((field) => {
      field.addEventListener("input", updateSpecialsDiscountPreview);
      field.addEventListener("change", updateSpecialsDiscountPreview);
    });
  updateSpecialsDiscountPreview();
  if (carouselForm) {
    // Added
